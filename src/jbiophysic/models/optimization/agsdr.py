...
class AGSDR:
    ...
    def compute_total_loss(
        self, state: dict[str, Any], empirical_target: dict[str, Any] | None = None
    ) -> float:
        l_rate = compute_rate_loss(state["rates"])

        if empirical_target is not None:
            l_gamma = compute_empirical_spectral_loss(
                empirical_target["psd"], state["psd"], empirical_target["gamma_mask"]
            )
            l_beta = compute_empirical_spectral_loss(
                empirical_target["psd"], state["psd"], empirical_target["beta_mask"]
            )
        else:
        ...
        target_occupancy = (
            empirical_target.get("target_occupancy", 0.5) if empirical_target else 0.5
        )
        l_pharma_val = (occupancy - target_occupancy) ** 2
        l_pharma = jax.lax.select(has_drug > 0.5, l_pharma_val, 0.0)

        total = (
            self.lambdas["rate"] * l_rate
    ...
    def update_weights(
        self, weights: jnp.ndarray, grad: jnp.ndarray, g_clip: float = 5.0, g_max: float = 10.0
    ) -> jnp.ndarray:
