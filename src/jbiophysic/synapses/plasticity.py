...
def mcdp_correlation(pre: jnp.ndarray, post: jnp.ndarray, eps: float = 1e-8) -> jnp.ndarray:
    ...
    pre_z = (pre - jnp.mean(pre, axis=0, keepdims=True)) / (
        jnp.std(pre, axis=0, keepdims=True) + eps
    )
    post_z = (post - jnp.mean(post, axis=0, keepdims=True)) / (
        jnp.std(post, axis=0, keepdims=True) + eps
    )
