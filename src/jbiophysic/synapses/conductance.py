...
def conductance_current(
    g: jnp.ndarray, s: jnp.ndarray, v_mV: jnp.ndarray, e_rev_mV: float
) -> jnp.ndarray:
    """Return synaptic current in conductance units using `I = g*s*(V-E)`."""
    return g * s * (v_mV - e_rev_mV)


def exp_synapse_step(
    s: jnp.ndarray, spike_drive: jnp.ndarray, tau_ms: float, dt_ms: float, amplitude: float = 1.0
) -> jnp.ndarray:
